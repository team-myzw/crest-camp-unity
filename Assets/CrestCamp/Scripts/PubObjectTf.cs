...
namespace SIGVerse.ToyotaHSR
{
	...
	public class PubObjectTf : MonoBehaviour
	{
		...
		[HeaderAttribute("TargetObjects")]
		public List<string> targetObjectTags;

		//private List<UnityEngine.Transform> targetObjectTransforms;
		...
		private class TfInfo
		{
			...
			public void UpdateTransformForGlobal()
			{
				//UnityEngine.Vector3 pos = linkTransform.localPosition;
				//UnityEngine.Quaternion qua = linkTransform.localRotation;
				UnityEngine.Vector3 pos = linkTransform.position;
				UnityEngine.Quaternion qua = linkTransform.rotation;
		...
		private List<UnityEngine.Transform> GetObjectTransforms()
		{
			List<UnityEngine.Transform> targetObjectTransforms = new List<UnityEngine.Transform>();

			foreach (string targetObjectTag in this.targetObjectTags)
			{
				GameObject[] targetObjects = GameObject.FindGameObjectsWithTag(targetObjectTag);

				foreach (GameObject targetObject in targetObjects)
				{
					targetObjectTransforms.Add(targetObject.transform);
				}
			}

			return targetObjectTransforms;
		}
	}
}
